...
extern int init_udp_socket(node_t *node);
...
create_new_graph(char *topology_name) {
	...
	// strncpy() must add terminator '\0' manually
	graph->topology_name[31] = '\0';
	// &graph->node_list == &(graph->node_list)
...
create_graph_node(graph_t *graph, char *node_name) {
	...
	// new node in graph is &node->graph_glue
...
void dump_interface(interface_t *interface) {
	...
	printf(" Local Node : %s, Interface Name = %s, Nbr Node %s, cost = %u\n",
				 interface->if_name, interface->att_node->node_name,
