...
void init_udp_socket(node_t *node) {
  ...
  int udp_sock_fd = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP); // return an integer of file descriptor
  if (udp_sock_fd == -1) {
...
_pkt_receive(node_t *receving_node, char *pkt_with_aux_data, unsigned int pkt_size) {
  // pkt_with_aux_data: auxiliary data
...
/* Whenever a node receive a frame on its local interface from nbr node, pkt_receive(...) is invoked */
int pkt_receive(node_t *node, interface_t *interface, char *pkt, unsigned int pkt_size) {
  /* Make room in the packet buffer by shifting the data towards right so that 
   * tcp/ip stack can append more headers to the packet as required */
