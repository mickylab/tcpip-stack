...
pkt_buffer_shift_right(char *pkt, unsigned int pkt_size, unsigned int total_buffer_size){
  ...
  if (pkt_size * 2 > total_buffer_size) need_temp_memory = TRUE;
  if (need_temp_memory) {
    // calloc() mostly like malloc(), but calloc() can allocate zero memory
    temp = calloc(1, pkt_size);
    memcpy(temp, pkt, pkt_size);
    memset(pkt, 0, total_buffer_size); // set all pkt's data to 0
